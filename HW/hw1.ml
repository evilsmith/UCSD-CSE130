...
(* let _ = digitsOfInt (-3421)
  ...
  *)
  ...
  if n >= 10 then
  helper(sumList(digitsOfInt n)) count+1 
  else 
  count
  in helper n 0
  ...
  match n >= 10 with
  | true  -> digitalRoot (sumList (digitsOfInt n))
  | false -> n
